...
export default function AutoPostPage() {
  ...
  const loadTargets = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/post-targets`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json() as FacebookTarget[];
      setTargets(data);
  ...
  const startTask = useCallback(async () => {
    if (!message.trim() && !link.trim() && mediaFiles.length === 0) {
      toast.error("Nhập nội dung, link hoặc media trước khi đăng");
      return;
    }
    if (selectedTargets.length === 0) {
      toast.error("Chọn ít nhất một nơi đăng");
      return;
    }
    stopPolling();
    setTask(null);
    setRunning(true);
    try {
      const res = await fetch(`${API_BASE}/api/page-post-tasks`, {
        method: "POST",
        body: buildPostFormData(selectedTargets.map((target) => target.id), message, link, maxThreads, mediaFiles),
      });
      if (!res.ok) {
        const detail = await res.text();
        throw new Error(detail || `HTTP ${res.status}`);
      }
      const data = await res.json() as { task_id: string; total: number; status: string };
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-[9pt] mt-0.5" style={{ color: "var(--muted-foreground)" }}>
            Đăng text/link/ảnh/video lên Fanpage quản lý, Group và trang cá nhân đã login browser.
          </p>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={7}
              disabled={running}
              className="max-h-[260px] overflow-auto text-[9pt]"
            ...
            <div className="flex flex-wrap items-center gap-2">
              <label className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-md border px-3 text-[9pt]" style={{ borderColor: "var(--border)", color: "var(--foreground)" }}>
                ...
                <input
                  ...
                  disabled={running}
                  onChange={(event) => {
                    setMediaFiles((current) => mergeMediaFiles(current, Array.from(event.target.files ?? [])));
                    event.target.value = "";
                  }}
                />
              </label>
        ...
        <aside className="space-y-3">
          ...
          <div className="overflow-hidden rounded-md border" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
            ...
            <div className="max-h-[520px] overflow-auto">
              {loading ? (
                <EmptyState message="Đang tải nơi đăng..." />
              ) : targets.length === 0 ? (
                ...
                targets.map((target) => (
                  <label
                    key={target.id}
                    className={`flex flex-wrap items-start gap-3 border-b p-3 sm:flex-nowrap ${target.available ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}
...
function mediaFileKey(file: File) {
