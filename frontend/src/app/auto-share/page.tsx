...
export default function AutoSharePage() {
  ...
  const selectedTargets = useMemo(
    () => targets.filter((target) => selected.has(target.id)),
    [targets, selected],
  );
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [accountsRes, targetsRes] = await Promise.all([
        fetch(`${API_BASE}/api/facebook-accounts`),
        fetch(`${API_BASE}/api/share-targets`),
      ]);
      ...
      const accountData = await accountsRes.json() as FacebookAccount[];
      const targetData = await targetsRes.json() as ShareTarget[];
      setAccounts(accountData);
      setTargets(targetData);
      setSelectedAccountId((prev) => prev || accountData[0]?.id || "");
      setSelected((prev) => new Set(targetData.filter((target) => target.available && prev.has(target.id)).map((target) => target.id)));
    } catch (error) {
  ...
  const refreshTask = useCallback(async (taskId: string) => {
    const res = await fetch(`${API_BASE}/api/page-post-tasks/${taskId}`);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json() as TaskSummary;
    setTask(data);
    if (DONE_STATUSES.has(String(data.status))) {
      stopPolling();
    }
  }, [stopPolling]);
  ...
  const importTargets = useCallback(async (kind: "group" | "external_page") => {
    ...
    if (!selectedAccountId) {
      toast.error("Chọn Facebook Account trước");
      return;
    }
    if (!rawText.trim()) {
      toast.error(kind === "group" ? "Nhập danh sách group URL" : "Nhập danh sách page URL");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}${endpoint}`, {
        ...
        body: JSON.stringify({ facebook_account_id: selectedAccountId, raw_text: rawText }),
      });
      if (!res.ok) {
      ...
      if (kind === "group") setGroupText("");
      else setExternalPageText("");
      await loadData();
    } catch (error) {
  ...
  const startCampaign = useCallback(async () => {
    if (!sourceUrl.trim()) {
      toast.error("Nhập link bài nguồn trước");
      return;
    }
    if (selectedTargets.length === 0) {
      toast.error("Chọn ít nhất một target share");
      return;
    }
    stopPolling();
    setTask(null);
    setRunning(true);
    try {
      const createRes = await fetch(`${API_BASE}/api/share-campaigns`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: campaignName,
          ...
          message_snapshot: customMessage,
      ...
      const campaign = await createRes.json() as { id: string; targets: number };
      const startRes = await fetch(`${API_BASE}/api/share-campaigns/${campaign.id}/start`, { method: "POST" });
      if (!startRes.ok) {
        const detail = await startRes.text();
        throw new Error(detail || `HTTP ${startRes.status}`);
      }
      const data = await startRes.json() as { task_id: string; targets: number; status: string };
      ...
      pollRef.current = window.setInterval(() => {
        void refreshTask(data.task_id).catch((error) => {
          toast.error(error instanceof Error ? error.message : "Không đọc được task");
          stopPolling();
        });
      }, 1500);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-[9pt] mt-0.5" style={{ color: "var(--muted-foreground)" }}>
            Share/link bài nguồn sang Group, Fanpage quản lý hoặc Page public nếu Facebook UI cho phép.
          </p>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_190px]">
            ...
            <div className="space-y-2">
              ...
              <select
                value={mode}
                disabled={running}
                onChange={(event) => setMode(event.target.value as "share_link" | "custom_content")}
                className="h-8 w-full rounded-md border px-2 text-[9pt]"
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={6}
              disabled={running}
              className="max-h-[240px] overflow-auto text-[9pt]"
              placeholder="Nhập caption đăng kèm link nguồn"
            />
          </div>
        ...
        <aside className="space-y-4">
          <section className="space-y-3">
            ...
            <div className="space-y-3 rounded-md border p-3" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
              <div className="space-y-2">
                <Label className="text-[9pt] font-medium" style={{ color: "var(--muted-foreground)" }}>
                  Facebook Account dùng để share
                </Label>
                <select
                ...
                >
                  {accounts.length === 0 ? (
                    <option value="">Chưa có account</option>
                  ) : accounts.map((account) => (
                    <option key={account.id} value={account.id}>
                      {account.name || account.uid} · {account.browser_status}
                    </option>
                  ))}
                </select>
              </div>
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  rows={4}
                  className="max-h-[140px] overflow-auto text-[9pt]"
                  placeholder="Mỗi dòng một link group"
                  disabled={running}
                />
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  value={externalPageText}
                  onChange={(event) => setExternalPageText(event.target.value)}
                  rows={4}
                  className="max-h-[140px] overflow-auto text-[9pt]"
                  placeholder="Mỗi dòng một link page bên ngoài"
                  disabled={running}
                />
            ...
            <div className="overflow-hidden rounded-md border" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
              <div className="flex flex-wrap items-center justify-between gap-2 border-b p-3" style={{ borderColor: "var(--border)" }}>
                ...
                <Button
                  ...
                  onClick={() => setSelected(new Set(targets.filter((target) => target.available).map((target) => target.id)))}
                  disabled={running || targets.every((target) => !target.available)}
                >
              ...
              <div className="max-h-[520px] overflow-auto">
                {loading ? (
                  <EmptyState message="Đang tải target..." />
                ) : targets.length === 0 ? (
                  <EmptyState message="Chưa có target. Hãy sync Fanpage hoặc import Group/Page ngoài." />
                ) : (
                  targets.map((target) => (
                    <label
                      key={target.id}
                      className={`flex flex-wrap items-start gap-3 border-b p-3 sm:flex-nowrap ${target.available ? "cursor-pointer" : "cursor-default opacity-80"}`}
                      style={{ borderColor: "var(--border)" }}
                      title={target.reason || ""}
                    >
                      <Checkbox
                        checked={selected.has(target.id)}
                        disabled={running || !target.available}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {target.reason && (
                          <div className="truncate text-[8pt]" style={{ color: "var(--muted-foreground)" }}>{target.reason}</div>
                        )}
                      </div>
                      {target.type !== "page" && (
                        <Button
